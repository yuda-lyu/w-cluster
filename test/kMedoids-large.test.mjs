...
describe(`k-medoids large`, function() {

    this.timeout(60000)

    it(`should perfectly recover 6 well-separated numeric blobs (n=120, usePCA:false)`, async function() {
        let G = 6
        let m = 20
        let n = G * m
        let { data, truth } = genBlobs(G, m, 4, 100, 5, 42)
        let r = await WCluster.cluster(data, { mode: 'k-medoids', kNumber: G, usePCA: false, useMethod: 'simple' })
    ...
    it(`should perfectly recover 6 binary blobs with Jaccard distance (n=120, D=120, usePCA:false)`, async function() {
        let G = 6
        let m = 20
        let n = G * m
        let { data, truth } = genBinaryBlobs(G, m, 120, 18, 3, 42)
        let r = await WCluster.cluster(data, { mode: 'k-medoids', kNumber: G, usePCA: false, funDist: jaccard, useMethod: 'simple' })
        assert.strict.strictEqual(coversAll(r.ginds, n), true)
        assert.strict.strictEqual(r.ginds.length, G)
        assert.strict.strictEqual(purity(r.ginds, truth), 1)
    })

    it(`should be deterministic on larger data (same ginds on repeated runs)`, async function() {
        let G = 6
        let m = 20
        let { data } = genBlobs(G, m, 4, 100, 5, 42)
        let r1 = await WCluster.cluster(data, { mode: 'k-medoids', kNumber: G, usePCA: false, useMethod: 'simple' })
        let r2 = await WCluster.cluster(data, { mode: 'k-medoids', kNumber: G, usePCA: false, useMethod: 'simple' })
